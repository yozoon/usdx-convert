#!/usr/bin/python
###############################################################################
#    Script to convert the output of ss_extract to ultrastar .txt format      #
#                            (c) yozoon 2017                                  #
###############################################################################
import untangle
import os 
...
def print_help():
    print 'Usage: ./convert.py -i <path/to/ss_extract-output-directory>'
...
def convert_media(song):
    ...
    pipe.communicate() # Wait for sp to finish
    # Remove xml file
    command = [ 'rm', song + os.path.sep + 'notes.xml']
    pipe = sp.Popen(command, stdout=FNULL, stderr=sp.STDOUT)
    pipe.communicate() # Wait for sp to finish
    # Remove OGG file
    command = [ 'rm', song + os.path.sep + 'music.ogg']
    pipe = sp.Popen(command, stdout=FNULL, stderr=sp.STDOUT)
    pipe.communicate() # Wait for sp to finish


def convert(song, index, length):
    print "--------------------------------"
    print bcolors.BOLD + "Converting file " + str(index+1) + " of " + str(length) + bcolors.ENDC + " (" + str(float(index+1)/float(length) * 100.0) + "%)"
    backup_files(song)
    convert_xml(song)
    convert_media(song)
    print " "
...
def select_songs(songlist):
    ...
    if select == '\n':
        ...
        for song in songlist:
            convert(song, count, len(songlist))
            count += 1
    else:
        try:
            int(select)
            ...
            if select >= 0 and select < len(songlist):
                convert(songlist[select], select, len(songlist))
            else :
